...
import { mapTrackerRowToTracker, mapTrackerToTrackerRow } from '@/lib/api/trackers/mappers';
...
export async function createTracker(input: CreateTrackerInput): Promise<Tracker> {
  ...
  const { data, error, status } = await supabase.from('trackers').insert(mapTrackerToTrackerRow(input)).eq('user_id', userData.user.id).select('*').single();
